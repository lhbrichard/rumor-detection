...
def train_model(num,treeDic, x_test, x_train,lr, weight_decay,patience,n_epochs,batchsize,dataname,modelname,iter):
    model = Net(5000,64,64).to(device)
    pth = modelname+dataname+'.m'
    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data in tqdm_train_loader:
            ...
            if modelname in ['GAE','VGAE']:
                loss = F.nll_loss(out_labels, Batch_data.y)+model.loss()
            elif modelname == 'GCN':
        ...
        early_stopping(np.mean(temp_val_losses), np.mean(temp_val_Acc_all), np.mean(temp_val_Acc1),
                       ...
                       np.mean(temp_val_F1),
                       np.mean(temp_val_F2), model, modelname, "weibo")
